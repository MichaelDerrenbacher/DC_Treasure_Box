...
float voltage[] = {-10,-9,-8,-7,-6,-5.9,-5.8,-5.7,-5.6,-5.5,-5.45,-5.4,-5.35,-5.3,-5.25,-5.2,-5.15,-5.1,-5.05,-5,-4.95,-4.9,-4.85,-4.8,-4.75,-4.7,-4.65,-4.6,-4.55,-4.5,-4.4,-4.3,-4.2,-4.1,-4,-3.5,-3,-2.5,-2,-1.5,-1,-0.5,0};

float PS_OUT[] = {5,5,5,5};
...
void main(void)
  {
    ...
    */
    ...
    value = (20.562465 * -2.0) + 1967.44939;   // - value
    ...
    output_DAC7565(0x07FF, SMU_A01); // V_f
    output_DAC7565(0x0830, SMU_A02); // V_c
    ...
    inter = (-5 * 0.995501) + 0.007848; // out 2
    ...
    inter = ((voltage[test]) * 0.99525) + 0.00617; // out 3
    value = ((inter + 10) * (65535 / 20));
    if (value > 65535)
    {
        value = 65535;
    ...
    */
    ...
    while(1)
    {
        ...
        {
            value = (PS_OUT[main_loop] * DAC_MAX) / (GAIN * DAC_REF);
           ...
           while(!finish)
           {
               ...
               if(voltage[test]*PS_OUT[main_loop] < 0)
               {
...
void init_uart(void)
{
    ...
    EUSCI_A0->CTLW0 |= (EUSCI_A_CTLW0_PEN        // odd parity
                    ...
                    | EUSCI_A_CTLW0_MODE_0       // MSB first
